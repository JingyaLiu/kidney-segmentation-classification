...
def load_model(model_path, device, num_classes=5, num_slices=3, gin_channels=4, lin_channels=4):
...
def process_batch(batch, device):
    """Extract and prepare batch data for inference."""
    if isinstance(batch, dict):
        if 'kidney_tumor_roi' in batch:
            ...
            kidney_tumor_roi = tumor_roi = images
    else:
        images, labels = batch
    ...
    return kidney_tumor_roi, tumor_roi, labels
...
def ensemble_predict(models, data_loader, device, num_classes=5, use_masks=True):
    """Perform ensemble prediction using models."""
    all_predictions = []
    all_probabilities = []
    all_labels = []
    
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Ensemble Inference"):
            kidney_tumor_roi, tumor_roi, labels = process_batch(batch, device)
            ...
            for model in models:
                if model is not None:
                    ...
                    fold_predictions.append(predicted.cpu().numpy())
...
def apply_threshold(probabilities, threshold=0.5, target_class=0):
    ...
    binary_predictions = (target_probs >= threshold).astype(int)
    
    return binary_predictions, target_probs
...
def calculate_metrics(y_true, y_pred, y_prob, class_names):
    ...
    sens_spec = calculate_sensitivity_specificity(y_true, y_pred, class_names)
...
def plot_class_probabilities(probabilities, labels, class_names, output_path):
    ...
    for i, class_name in enumerate(class_names):
        ...
        axes[i].hist(class_probs, bins=50, alpha=0.7, edgecolor='black')
...
def main():
    parser = argparse.ArgumentParser(description='Ensemble inference for kidney tumor classification')
    
    parser.add_argument('--data_dir', type=str, default='./processed_data_with_smooth_masks_augmented',
    ...
    parser.add_argument('--num_cases', type=int, default=100,
                       help='Number of cases to process (0 for all)')
    ...
    if args.num_cases > 0:
        dataset.cases_data = dataset.cases_data[:args.num_cases]
    ...
    predictions, probabilities, labels = ensemble_predict(models, data_loader, device, args.num_classes, args.use_masks)
    
    print(f"Applying threshold {args.clear_cell_threshold} for Clear Cell RCC detection...")
    binary_predictions, clear_cell_probs = apply_threshold(probabilities, args.clear_cell_threshold, target_class=0)
    
    print("Calculating metrics...")
    metrics = calculate_metrics(labels, predictions, probabilities, class_names)
    
    clear_cell_accuracy = accuracy_score((labels == 0).astype(int), binary_predictions)
    ...
    np.save(output_dir / 'binary_predictions.npy', binary_predictions)
    np.save(output_dir / 'clear_cell_probabilities.npy', clear_cell_probs)
    ...
    for i in range(len(labels)):
        result = {
            ...
            'class_probabilities': {
                class_names[j]: float(probabilities[i, j]) for j in range(len(class_names))
            }
        }
    ...
    plot_class_probabilities(probabilities, labels, class_names, output_dir / 'class_probabilities.png')
    
    plt.figure(figsize=(10, 6))
    plt.hist(clear_cell_probs, bins=50, alpha=0.7, edgecolor='black')
    ...
    plt.title('Clear Cell RCC Probability Distribution')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'clear_cell_probability_distribution.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\nInference completed successfully!")
    print(f"Results saved to: {output_dir}")
    print(f"Visualizations: confusion_matrix.png, class_probabilities.png, clear_cell_probability_distribution.png")
    print("="*60)
