...
class partial_DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=0, weight_dice=1,
                 log_dice=False, ignore_label=1, ignore_index=0):
        """
        ...
        :param weight_dice:
        """
        super(partial_DC_and_CE_loss, self).__init__()
    ...
    def forward(self, net_output, target):
        ...
        # get the mask for partial annos
        mask_partial = target.clone().ne_(self.ignore_index) # ignore the slice with no annos
        mask_partial.requires_grad = False
        # mask out input/target so that the gradient is zero where on the mask
        net_output = net_output * mask_partial
        target = target * mask_partial
