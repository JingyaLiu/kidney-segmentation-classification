...
def predict_from_folder(input_folder: str,
    ...
    # Load model - try different possible paths
    possible_paths = [
        os.path.join(os.environ['RESULTS_FOLDER'], 'nnUNet', model, task_name),
        os.path.join(os.environ['RESULTS_FOLDER'], 'nnUNet', task_name, model),
        os.path.join(os.environ['RESULTS_FOLDER'], task_name, model)
    ]
    ...
    if not model_path:
        print(f"Model path not found. Tried:")
        for path in possible_paths:
            print(f"  - {path}")
        return
    ...
    # Find the trainer directory
    trainer_dirs = [d for d in os.listdir(model_path) if os.path.isdir(os.path.join(model_path, d)) and 'nnUNetTrainer' in d]
    
    if not trainer_dirs:
        print(f"No trainer directories found in {model_path}")
        return
    
    # Use the first available trainer directory
    trainer_dir = trainer_dirs[0]
    print(f"Using trainer directory: {trainer_dir}")
    
    # Load trainer and model
    trainer = restore_model(os.path.join(model_path, trainer_dir), 
